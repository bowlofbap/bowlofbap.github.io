...
var game = new Phaser.Game(600, 800, Phaser.AUTO,'',null,true), Tutorial = function() {
	screenWidth = 600;
	screenHeight = 800;
	pageNumber = 0;
	SIZEMAP = {
...
Tutorial.prototype = {
	...
	showPage: function(page){
		if (page == 0){
			...
			text = game.add.text(game.world.width/2,game.world.height/4, "		The objective is to add up numbers in a chain to create a perfect square (4, 9, 16, etc).\n\n Add numbers to your chain with ENTER and W/A/S/D. Press ESC to cancel your chain.", {font: SIZEMAP['body']*game.world.width+'px TestFont', fill: '#ffffff', wordWrap: true, wordWrapWidth: game.world.width});
			text.anchor.setTo(0.5,0.5);
			textLayer.addChild(text);

		}else if (page == 2){
			text = game.add.text(game.world.width/2,game.world.height/4, "Your chain normally has a maximum of 5 tiles but you can increase it every time you get 5 perfect squares.", {font: SIZEMAP['body']*game.world.width+'px TestFont', fill: '#ffffff', wordWrap: true, wordWrapWidth: game.world.width});
			text.anchor.setTo(0.5,0.5);
			textLayer.addChild(text);
		}else if (page == 3){
